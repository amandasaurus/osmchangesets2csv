...
async fn main() -> Result<()> {
    ...
    let columns: Vec<Column> = args.columns
        .split(',')
    ...
    let mut reader = ChangesetReader::from_bz2_reader(read_progress::ReaderWithSize::from_path(args.input)?);
    ...
    while let Some(c) = reader.next_changeset()? {
        ...
        for col in &columns {
            match col {
                ...
                Column::Closed(_title) => output.write_field(c.closed.clone().map_or("".to_string(), |cl| cl.to_iso_string()))?,
                Column::Uid(_title) => output.write_field(c.uid.clone().map_or("".to_string(), |x| x.to_string()))?,
                Column::User(_title) => output.write_field(c.user.clone().map_or("".to_string(), |x| x.to_string()))?,
                ...
                Column::CommentsCount(_title) => output.write_field(c.comments_count.to_string())?,

                Column::Tag(tag, _title) => output.write_field(c.tag(tag).unwrap_or("").to_string())?,
            }
        }
